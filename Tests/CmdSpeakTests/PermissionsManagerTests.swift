...
@Suite("CmdSpeakController State Tests")
struct CmdSpeakControllerStateTests {

    @Test("State enum cases are distinct")
    func testStateDistinctness() {
        let states: [CmdSpeakController.State] = [
            .idle,
            .listening,
            .processing,
            .injecting,
        ...
        // All states should have different string representations
    ...
    func testErrorStateMessage() {
        let errorState = CmdSpeakController.State.error("Test error message")
...
@Suite("Model Load Progress Tests Extended")
struct ModelLoadProgressExtendedTests {

    @Test("All stages have distinct names")
    func testStageDistinctness() {
        ...
        let descriptions = stages.map { String(describing: $0) }
        let uniqueDescriptions = Set(descriptions)
        #expect(uniqueDescriptions.count == stages.count)
    }

    @Test("Progress values are clamped to valid range")
    func testProgressRange() {
        let progress1 = ModelLoadProgress(stage: .downloading, progress: 0.0, message: "Start")
        #expect(progress1.progress >= 0.0)

        let progress2 = ModelLoadProgress(stage: .ready, progress: 1.0, message: "Done")
        #expect(progress2.progress <= 1.0)

        let progress3 = ModelLoadProgress(stage: .downloading, progress: 0.5, message: "Half")
        #expect(progress3.progress >= 0.0 && progress3.progress <= 1.0)
    }

    @Test("Message is preserved")
    func testMessagePreserved() {
        let message = "Downloading: 500 / 954 MB"
        let progress = ModelLoadProgress(stage: .downloading, progress: 0.52, message: message)
        #expect(progress.message == message)
    }
}
